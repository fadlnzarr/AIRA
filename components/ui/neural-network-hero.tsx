...
if (typeof window !== 'undefined') {
    ...
    // SplitText is a Club/Premium plugin, might not be available in standard gsap. 
    // If the user provided code uses it, they probably have it or it's a mock/trial.
    // Standard gsap installation does NOT include SplitText. 
    // I will check if I can use it or if I need to fallback.
    // However, I am tasked to "Copy-paste this component". I will paste it as is.
    try {
        gsap.registerPlugin(SplitText);
    } catch (e) {
        console.warn("SplitText plugin not found/registered. Animation might fail if not available.");
    }
}
...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[5]
    + vec4(6.6818056, 12.522166, -3.7075126, -4.101023);
    ...
    * buf[7]
    ...
    // Output color: Mix between White background and Red pattern
    // buf[0].x is the pattern value (0.0 to 1.0)
    // We want 0.0 -> White, 1.0 -> Red
    vec3 colorRed = vec3(0.85, 0.08, 0.08); // #d92514 - Approximate GLSL conversion
    vec3 colorWhite = vec3(1.0, 1.0, 1.0);
    
    // Use the pattern (buf[0].x) to mix. 
    // Experimentation: pattern might be inverted, we can swap if needed.
    vec3 finalColor = mix(colorWhite, colorRed, buf[0].x);
...
export function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="bg-black absolute inset-0 -z-10 w-full h-full" aria-hidden>
            ...
            {/* Removed dark gradient overlays for clean white background */}
        </div>
    );
}
...
export default function Hero({
    ...
    useGSAP(
        () => {
            ...
            document.fonts.ready.then(() => {
                // Fallback for SplitText if it's not present (Premium Plugin)
                if (gsap.plugins.SplitText) {
                    ...
                    gsap.set(headerRef.current, { autoAlpha: 0, y: 20, filter: 'blur(10px)' });
                    const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
                    if (badgeRef.current) tl.to(badgeRef.current, { autoAlpha: 1, y: 0, duration: 0.5 }, 0.0);
                    tl.to(headerRef.current, { autoAlpha: 1, y: 0, filter: 'blur(0px)', duration: 0.9 }, 0.1);
                }
                ...
                // Continue timeline
                const tl = gsap.timeline({ defaults: { ease: 'power3.out' }, delay: 0.5 }); // Add delay to sync with above if split text is used via existing tl variable?
                // Actually, let's just make a unified flow.
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
            ...
            {/* Content Container - kept for reference if used directly, though Home.tsx uses its own content */}
            <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16 pointer-events-none">
                {/* 
                   Note: Home.tsx renders its own content over this background. 
                   This component's internal content (HeroProps) might not be visible if overlayed by Home.tsx logic.
                   However, keeping it intact as requested. 
                */}
            </div>
        </section>
    );
